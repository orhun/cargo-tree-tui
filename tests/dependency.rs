...
fn known_dependency_present() {
    ...
    let has_ratatui = tree.crate_nodes().iter().any(|&id| {
        tree.node(id)
...
fn group_nodes_have_valid_kind() {
    ...
    for (idx, node) in tree.nodes.iter().enumerate() {
        if let DependencyNode::Group(group) = node {
            ...
            assert!(
                tree.parent_of(group_id).is_some(),
...
fn dev_dependencies_under_group() {
    ...
    let pa_node = tree.crate_nodes().iter().find(|&&id| {
        tree.node(id)
            .map(|n| n.display_name() == "pretty_assertions")
            .unwrap_or(false)
    });

    if let Some(&pa_id) = pa_node {
        let parent_id = tree
            .parent_of(pa_id)
            .expect("pretty_assertions should have a parent");
        let parent = tree.node(parent_id).unwrap();
        assert!(
            parent.is_group(),
            "dev dependency should be nested under a group node"
        );
        let group = parent.as_group().unwrap();
        assert_eq!(
            group.kind,
            DependencyType::Dev,
            "pretty_assertions should be under [dev-dependencies]"
        );
    } else {
        panic!("pretty_assertions should be in the tree as a dev dependency");
    }
}
